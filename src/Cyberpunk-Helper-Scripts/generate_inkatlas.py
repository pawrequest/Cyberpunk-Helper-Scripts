...
try:
    ...
    print('')
    print(
        'you need to install PILLOW (Python Imaging Library) by copy pasting the following into the command line:'
    )
    print('')
    ...
    print(
        'https://wiki.redmodding.org/cyberpunk-2077-modding/for-mod-creators/modding-guides/everything-else/running-python-scripts'
    )
    print(
        '-----------------------------------------------------------------------------------------------'
    )
...
def main():
    ...
    atlas_name = os.getenv('INKATLAS_OUTPUT_NAME') or input(
        '    Enter the name for your new inkatlas file (without extension): '
    )
    ...
    total_height = sum(
        max(image_data['image'].height for image_data in row) for row in grid
    )
    ...
    for row in grid:
        row_width = sum(
            image_data['image'].width for image_data in row
        )  # Calculate the total width of images in the current row
        total_width = max(
            total_width, row_width
        )  # Update the total width if the current row width is greater
...
def get_source_dir_and_pngs():
    ...
    while True:
        icon_folder = icon_folder or input(
            '    Enter the path to the folder containing the PNG files you want to combine into an inkatlas: '
        )
        ...
        if not os.path.exists(icon_folder):
            ...
            print('INKATLAS GENERATOR INPUT: ')
            icon_folder = (
                None  # Reset the input in case env var leads to an invalid path
            )
            continue
...
def get_output_dir():
    ...
    while True:
        output_folder = output_folder or input(
            '    Enter the path to output the raw inkatlas files for you to import in Wolvenkit: '
        )
        # Check for quotes which will break the input path
        if (output_folder.startswith('"') or output_folder.startswith("'")) and (
            output_folder.endswith('"')
            or output_folder.endswith("'")
            or output_folder.endswith('\\')
        ):
            # Remove the quotes
        ...
        if 'raw' in output_folder:
            ...
            print('')
            print(
                f'    Entered path {output_folder} does not seem to be within a Wolvenkit project.'
            )
            print('')
            print(
                '-----------------------------------------------------------------------------------------------'
